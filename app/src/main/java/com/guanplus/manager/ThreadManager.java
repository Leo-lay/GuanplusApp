...
public class ThreadManager
{
	...
	/** 获取�?个用于执行长耗时任务的线程池，避免和短�?�时任务处在同一个队列�?�阻塞了重要的短耗时任务，�?�常用来联网操作 */
	...
	/** 获取�?个用于执行短耗时任务的线程池，避免因为和耗时长的任务处在同一个队列�?�长时间得不到执行，通常用来执行本地的IO/SQL */
	...
	/** 获取�?个单线程池，�?有任务将会被按照加入的顺序执行，免除了同步开�?的问�? */
	...
	public static class ThreadPoolProxy
	{
		...
		public synchronized void execute(Runnable run)
		{
			...
			if (mPool == null || mPool.isShutdown())
			{
				...
				// 当线程池中的线程数目等于mCorePoolSize，将会把任务放入任务队列BlockingQueue�?
				// 当BlockingQueue中的任务放满了，将会创建新的线程去执行，
				// 但是当�?�线程数大于mMaximumPoolSize时，将会抛出异常，交给RejectedExecutionHandler处理
				// mKeepAliveTime是线程执行完任务后，且队列中没有可以执行的任务，存活的时间，后面的参数是时间单位
				// ThreadFactory是每次创建新的线程工�?
		...
		/** 平缓关闭单任务线程池，但是会确保�?有已经加入的任务都将会被执行完毕才关�? */
